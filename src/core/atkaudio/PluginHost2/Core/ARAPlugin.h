...
public:
    ...
    {
        // TODO: the ARA interface defines numSamples as int64. We should do multiple reads if necessary with the
        // reader.
    ...
    MemoryMappedAudioFormatReader& getFormatReader() const
    {
        return *formatReader;
    }

    auto getPluginRef() const
    {
...
public:
    PlaybackRegion(ARA::Host::DocumentController& dc, RegionSequence& s, AudioModification& m, FileAudioSource& source)
        : sequence(s)
        , audioSource(source)
...
public:
    ...
    createAudioReaderForSource(ARA::ARAAudioSourceHostRef audioSourceHostRef, bool use64BitSamples) noexcept override
    {
    ...
    ) noexcept override
    {
        ...
        if (use64BitSamples)
            return audioSource
                ->readAudioSamples(reinterpret_cast<double* const*>(buffers), samplePosition, samplesPerChannel);

        return audioSource
            ->readAudioSamples(reinterpret_cast<float* const*>(buffers), samplePosition, samplesPerChannel);
    }
...
public:
    ...
    void notifyAudioSourceContentChanged(
        ARA::ARAAudioSourceHostRef audioSourceHostRef,
        const ARA::ARAContentTimeRange* range,
        ARA::ContentUpdateScopes scopeFlags
    ) noexcept override
    {
...
public:
    ...
    void resized() override
    {
        ...
        fb.items = {
            juce::FlexItem(goToStartButton).withMinWidth(100.0f).withMinHeight((float)buttonsArea.getHeight()),
    ...
    void update()
    {
        ...
        if (currentAudioSource.existsAsFile())
        {
            ...
            audioSourceLabel.setText(currentAudioSource.getFullPathName(), NotificationType::dontSendNotification);
        }
        else
        {
...
{
public:
    class ARATestHost final
        : public HostPlaybackController
    ...
    public:
        ...
        public:
            ...
            void resized() override
            {
                audioSourceComponent.setBounds(getLocalBounds());
            }
            ...
            : instance(instanceIn)
        {
            if (instance.inner->getPluginDescription().hasARAExtension)
            {
                ...
                createARAFactoryAsync(
                    *instance.inner,
                    [this](ARAFactoryWrapper araFactory) { init(std::move(araFactory)); }
                );
            }
        }
            ...
            if (araFactory.get() != nullptr)
            {
                ...
                if (documentController != nullptr)
                {
                    ...
                    const auto plugInExtensionInstance =
                        documentController->bindDocumentToPluginInstance(*instance.inner, allRoles, allRoles);
        ...
        void afterProcessBlock(int numSamples)
        {
            ...
            if (isPlayingNow)
            {
                ...
                // value. We should gain control over calling the AudioProcessorGraphMT's processBlock() calls so
        ...
        AudioPluginInstance& getAudioPluginInstance()
        {
            return instance;
        }
    ...
    private:
        ...
        */
        ...
        {
        public:
            explicit ScopedPluginDeactivator(ARAPluginInstanceWrapper& inst)
                : instance(inst)
            {
            ...
            {
                if (instance.prepareToPlayParams.isValid)
                    instance.inner->prepareToPlay(
                        instance.prepareToPlayParams.sampleRate,
    ...
    void setPlayHead(AudioPlayHead* p) override
    {
        ignoreUnused(p);
    }
...
private:
    ...
    struct PrepareToPlayParams
    {
        ...
        PrepareToPlayParams(double sampleRateIn, int samplesPerBlockIn)
            ...
            , samplesPerBlock(samplesPerBlockIn)
        {
        }

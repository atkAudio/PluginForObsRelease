...
public:
    ...
    static void destroyOwnedObjectOnMessageThread(OwnedType*& object, int timeoutMs = 200)
    {
        auto* ptr = std::exchange(object, nullptr);
        if (ptr == nullptr)
            return;

        destroyOnMessageThread([ptr]() { delete ptr; }, timeoutMs);
    }

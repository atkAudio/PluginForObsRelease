...
atk::DeviceIo2::~DeviceIo2()
{
    ...
    auto* settingsWindowPtr = settingsWindow.release();
    auto* moduleDeviceManagerPtr = moduleDeviceManager.release();
    auto* deviceManagerPtr = deviceManager.release();

    atk::atkAudioModule::destroyOnMessageThread(
        [settingsWindowPtr, moduleDeviceManagerPtr, deviceManagerPtr]()
        {
            if (settingsWindowPtr != nullptr)
            {
                settingsWindowPtr->setVisible(false);
                if (settingsWindowPtr->isOnDesktop())
                    settingsWindowPtr->removeFromDesktop();
            }

            if (deviceManagerPtr != nullptr)
                deviceManagerPtr->closeAudioDevice();

            if (moduleDeviceManagerPtr != nullptr)
            {
                moduleDeviceManagerPtr->cleanup();
                delete moduleDeviceManagerPtr;
            }

            delete settingsWindowPtr;
            delete deviceManagerPtr;
        }
    );
}

void atk::DeviceIo2::handleAsyncUpdate()
{
...
void atk::DeviceIo2::updateChannelInfoOnMessageThread(int numChannels)
{
...
void atk::DeviceIo2::prepareOutputDelay(int numChannels, int numSamples, double sampleRate)
{
...
void atk::DeviceIo2::applyOutputDelay(
...
void atk::DeviceIo2::getState(std::string& s)
{
    ...
    s = rootElement->toString().toStdString();
}
...
juce::Component* atk::DeviceIo2::getWindowComponent()
{
    if (settingsWindow == nullptr)
    {
        ...
        settingsWindow = std::make_unique<SettingsWindow>(&audioClient, deviceManager.get(), currentNumChannels);
        if (auto* audioComponent =
                dynamic_cast<atk::AudioServerSettingsComponent*>(settingsWindow->getContentComponent()))
        {
            juce::StringArray channelNames;
...
juce::Component* atk::DeviceIo2::createEmbeddableSettingsComponent()
{
    ...
    audioComponent->setDeviceManager(deviceManager.get());

...
#include <atkaudio/PluginHost2.h>
#include <inttypes.h>
#include <math.h>
...
static inline obs_source_t* get_sidechain(struct pluginhost2_data* ph)
{
...
{
    struct pluginhost2_data* ph = (struct pluginhost2_data*)param;
...
static void pluginhost2_tick(void* data, float seconds)
{
    struct pluginhost2_data* ph = (struct pluginhost2_data*)data;
    char* new_name = NULL;
...
static struct obs_audio_data* pluginhost2_filter_audio(void* data, struct obs_audio_data* audio)
{
    ...
    ph->pluginHost2.process(ph->pointersToProcess.data(), (int)ph->num_channels, num_samples, (double)ph->sample_rate);

    return audio;
}

static void pluginhost2_defaults(obs_data_t* s)
{

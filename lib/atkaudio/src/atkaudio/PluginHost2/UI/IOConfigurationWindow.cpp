...
private:
    ...
    Component* refreshComponentForCell(int, int columnId, bool, Component* existingComponentToUpdate) override
    {
        ...
        textButton->setConnectedEdges(
            Button::ConnectedOnLeft | Button::ConnectedOnRight | Button::ConnectedOnTop | Button::ConnectedOnBottom
        );
            ...
            if (auto* bus = plugin->getBus(isInput, currentBus))
            {
                ...
                for (auto i = 1; i <= AudioChannelSet::maxChannelsOfNamedLayout; ++i)
                {
                    for (const auto& set : AudioChannelSet::channelSetsWithNumberOfChannels(i))
                    {
                        ...
                        {
                            menu->addItem(
                                PopupMenu::Item{set.getDescription()}.setAction(
                                                                         [this, set] { applyBusLayout(set); }
                                ).setID(itemId)
                            );
                        }
...
void IOConfigurationWindow::update()
{
    ...
    if (auto* graph = getGraph())
        if (nodeID != AudioProcessorGraphMT::NodeID())
...
AudioProcessorGraphMT::NodeID IOConfigurationWindow::getNodeID() const
{
    if (auto* graph = getGraph())

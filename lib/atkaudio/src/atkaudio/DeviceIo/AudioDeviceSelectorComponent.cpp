...
struct SimpleDeviceManagerInputLevelMeter final
    : public Component
    , public Timer
{
...
static void
drawTextLayout(Graphics& g, Component& owner, StringRef text, const Rectangle<int>& textBounds, bool enabled)
{
...
//==============================================================================
class AudioDeviceSelectorComponent::MidiInputSelectorComponentListBox final
    : public ListBox
...
//==============================================================================
class AudioDeviceSelectorComponent::MidiOutputSelector final
    : public Component
...
public:
        ...
        : deviceManager(dm)
    {
        ...
        selector.onChange = [&]
        {
            ...
            const auto deviceId =
                selectedId == -1 ? String{} : MidiOutput::getAvailableDevices()[selectedId - 1].identifier;
...
private:
    ...
    void changeListenerCallback(ChangeBroadcaster*) final
    {
        updateListOfDevices();
    }
...
public:
    ...
    void updateConfig(bool updateOutputDevice, bool updateInputDevice, bool updateSampleRate, bool updateBufferSize)
    {
        ...
        if (updateOutputDevice || updateInputDevice)
        {
            if (outputDeviceDropDown != nullptr)
                config.outputDeviceName =
                    outputDeviceDropDown->getSelectedId() < 0 ? String() : outputDeviceDropDown->getText();

            if (inputDeviceDropDown != nullptr)
                config.inputDeviceName =
                    inputDeviceDropDown->getSelectedId() < 0 ? String() : inputDeviceDropDown->getText();
        ...
        if (error.isNotEmpty())
            messageBox = AlertWindow::showScopedAsync(
                MessageBoxOptions()
    ...
    void toggleAdvancedSettings()
    {
        showAdvancedSettingsButton->setButtonText(
            (showAdvancedSettingsButton->getToggleState() ? "Hide " : "Show ") + String("advanced settings...")
        );
        resized();
    }
...
private:
    ...
    std::unique_ptr<Label> outputDeviceLabel, inputDeviceLabel, sampleRateLabel, bufferSizeLabel, inputChanLabel,
        outputChanLabel;
    ...
    void updateControlPanelButton()
    {
        ...
        {
            showUIButton =
                std::make_unique<TextButton>(TRANS("Control Panel"), TRANS("Opens the device's own control panel"));
    ...
    void updateOutputsComboBox()
    {
        if (setup.maxNumOutputChannels > 0 || !type.hasSeparateInputsAndOutputs())
        {
            if (outputDeviceDropDown == nullptr)
            {
                ...
                outputDeviceLabel = std::make_unique<Label>(
                    String{},
...
public:
    //==============================================================================
    class ChannelSelectorListBox final
        : public ListBox
        , private ListBoxModel
    {
    public:
    ...
    private:
        ...
        void flipEnablement(int row)
        {
            ...
            if (isPositiveAndBelow(row, items.size()))
            {
                ...
                {
                    BigInteger bits;
                    auto& original = (type == audioInputType ? config.inputChannels : config.outputChannels);
                ...
                auto numInputChannels = config.inputChannels.countNumberOfSetBits();
                auto numOutputChannels = config.outputChannels.countNumberOfSetBits();

                if (numInputChannels == 0 && numOutputChannels == 0)
                    setup.manager->initialise(0, 0, nullptr, true);
    ...
    , hideAdvancedOptionsWithButton(hideAdvancedOptionsWithButtonToUse)
{
    ...
    {
        midiInputsList = std::make_unique<MidiInputSelectorComponentListBox>(
            deviceManager,
        ...
        {
            bluetoothButton =
                std::make_unique<TextButton>(TRANS("Bluetooth MIDI"), TRANS("Scan for bluetooth MIDI devices"));
...
void AudioDeviceSelectorComponent::resized()
{
    ...
    if (audioDeviceSettingsComp != nullptr)
    {
        ...
        audioDeviceSettingsComp->setBounds(
            r.removeFromTop(audioDeviceSettingsComp->getHeight()).withX(0).withWidth(getWidth())
        );
        r.removeFromTop(space);
    }
    ...
    if (midiInputsList != nullptr)
    {
        ...
        midiInputsList->setBounds(r.removeFromTop(
            midiInputsList->getBestHeight(jmin(itemHeight * 8, getHeight() - r.getY() - space - itemHeight))
        ));
        r.removeFromTop(space);
    }
    ...
    auto* topLevel = dynamic_cast<ResizableWindow*>(getTopLevelComponent());
    if (topLevel)
        topLevel->setSize(getWidth(), r.getY());
}
    ...
    if (audioDeviceSettingsComp == nullptr || audioDeviceSettingsCompType != deviceManager.getCurrentAudioDeviceType())
    {
            ...
            )[deviceTypeDropDown == nullptr ? 0 : deviceTypeDropDown->getSelectedId() - 1])
        {
            ...
            audioDeviceSettingsComp =
                std::make_unique<AudioDeviceSettingsPanel>(*type, details, hideAdvancedOptionsWithButton, *this);
